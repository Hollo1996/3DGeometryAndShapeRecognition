	...
	QMainWindow(), parent(parent), last_directory(".")
{
	...
	connect(viewer, SIGNAL(median(double)), this, SLOT(homeWorkMessage(double)));
...
void MyWindow::open() {
	auto filename =
		QFileDialog::getOpenFileName(this, tr("Open File"), last_directory,
			tr("Readable files (*.obj *.ply *.stl *.bzr *.bzrt *.bzrtp);;"
				"Mesh (*.obj *.ply *.stl);;"
				"Rectengular Bézier surface (*.bzr);;"
				"Triangular Bézier surface (*.bzrt);;"
				"Pair of Triangular Bézier surface (*.bzrtp);;"
				"All files (*.*)"));
	if (filename.isEmpty())
		return;
	...
	if (filename.endsWith(".bzr"))
		ok = viewer->openBezierRect(filename.toUtf8().data());
	else if (filename.endsWith(".bzrt"))
		ok = viewer->openBezierTri(filename.toUtf8().data());
	else if (filename.endsWith(".bzrtp"))
		ok = viewer->openBezierTriPair(filename.toUtf8().data());
	else
...
void MyWindow::save() {
	auto filename =
		QFileDialog::getSaveFileName(this, tr("Save File"), last_directory,
			tr("Rectengular Bézier surface (*.bzr);;"
				"Triangular Bézier surface (*.bzrt);;"
			));
	if (filename.isEmpty())
		return;
	...
	bool ok;
	if (filename.endsWith(".bzr"))
		ok = viewer->saveBezierRect(filename.toUtf8().data());
	else if (filename.endsWith(".bzrt"))
		ok = viewer->saveBezierTriPair(filename.toUtf8().data());
	else if (filename.endsWith(".bzrtp"))
		ok = viewer->saveBezierTri(filename.toUtf8().data());
	else QMessageBox::warning(this, tr("Cannot save file"), tr("Could not save file: ") + filename + ".");
...
void MyWindow::homeWorkMessage(double median) {

	auto dlg = std::make_unique<QDialog>(this);
	auto* hb = new QHBoxLayout;
	std::ostringstream strs;
	strs << median;
	std::string medianStr = strs.str();
	auto* text = new QLabel(tr("The median is: ") + tr(medianStr.data()));


	hb->addWidget(text);

	dlg->setWindowTitle(tr("Median size"));
	dlg->setLayout(hb);
